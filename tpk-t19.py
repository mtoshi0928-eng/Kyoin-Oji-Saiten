...
for i in range(1, 11):
    try:
        img_m = pygame.image.load(f"〇　透明{i}.png").convert_alpha()
        maru_images.append(pygame.transform.scale(img_m, (80, 80)))
        img_c = pygame.image.load(f"✖　透明{i}.png").convert_alpha()
...
try:
    se_btn_hover = pygame.mixer.Sound("カーソル移動 効果音.mp3")       
    se_btn_start = pygame.mixer.Sound("スタートボタン効果音.mp3")     
    se_btn_score_view = pygame.mixer.Sound("採点効果音.mp3")         
...
try:
    bg_image = pygame.image.load("教員 机6 昼.png").convert()
    bg_image = pygame.transform.scale(bg_image, (SCREEN_WIDTH, SCREEN_HEIGHT))
except:
    bg_image = None

try:
    img_unmarked = pygame.image.load("未採点1 透過.png").convert_alpha()
    img_unmarked = pygame.transform.scale(img_unmarked, (180, 180)) # 縦幅に合わせて少しコンパクト化
except: img_unmarked = None

try:
    img_marked = pygame.image.load("採点済み2 透過.png").convert_alpha()
...
def play_scene_bgm(scene_name):
    try:
        ...
        if scene_name == "TITLE" or scene_name == "RESULT":
            pygame.mixer.music.load("タイトル、リザルト BGM.mp3") 
            pygame.mixer.music.set_volume(0.4)       
            pygame.mixer.music.play(-1)              
        elif scene_name == "PLAY":
            pygame.mixer.music.load("プレイ画面 BGM.mp3")  
